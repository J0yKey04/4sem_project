/*
    ...
    macOS build:
        clang++ RainbowSimulation.cpp -std=c++11 -framework OpenGL -framework GLUT -o RainbowSimulation
        ./RainbowSimulation

    Linux build:
        g++ RainbowSimulation.cpp -std=c++11 -lGL -lGLU -lglut -o RainbowSimulation
        ./RainbowSimulation
...
static void drawRainbowCones() {
    ...
    std::vector<double> wavelengths;
    if (S.whiteLight) {
        for (int i = 0; i < 24; ++i) wavelengths.push_back(380.0 + i * (400.0 / 23.0));
    } else {
        wavelengths.push_back(S.wavelength);
    }
    ...
    for (size_t wi = 0; wi < wavelengths.size(); ++wi) {
        ...
        double centers[2] = { rainbowAnglePrimary(wl), rainbowAngleSecondary(wl) };
        for (int order = 0; order < 2; ++order) {
            if (order == 1 && S.internalReflections < 2) continue;
            double theta = rad(centers[order]);
            double alpha = order == 0 ? 0.85 : 0.36;
            glLineWidth(order == 0 ? 3.0f : 2.0f);
            glColor(c, alpha);
            glBegin(GL_LINE_STRIP);
            for (int i = 0; i <= 180; ++i) {
                double phi = rad(-90.0 + i * 180.0 / 180.0);
                Vec3 dir = normalize(antiSolar * std::cos(theta) + (u*std::cos(phi) + v*std::sin(phi)) * std::sin(theta));
    ...
    // Alexander's dark band as a transparent dark arc between primary and secondary.
    glLineWidth(8.0f);
    glColor4d(0.02, 0.02, 0.04, 0.28);
    for (double angle = 45.0; angle < 49.5; angle += 0.8) {
